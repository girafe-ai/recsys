...
def setup_qdrant(collection_name: str, port: int, emb_dim: int):
    ...
    client.create_collection(
        ...
        vectors_config=VectorParams(
            size=emb_dim, distance=Distance.COSINE, on_disk=True
        ),
    )
    return client
...
def upload_to_qdrant(
    client: QdrantClient, collection_name: str, embeddings: np.ndarray, batch_size: int
):
    for i in range(0, len(embeddings), batch_size):
        batch_vectors = embeddings[i : i + batch_size]

        points = [
            models.PointStruct(id=idx + i, vector=embedding.tolist())
        ...
        try:
            ...
            print(
                f"Uploaded batch {i//batch_size + 1}/{(len(embeddings)-1)//batch_size + 1}"
            )
...
def search_qdrant(
    client: Any, query_embedding: np.ndarray, collection_name: str, k: int = 5
):
...
def main(conf: DictConfig):
    ...
    qdrant_client = setup_qdrant(conf["collection_name"], conf["port"], conf["emb_dim"])

    print("Uploading to Qdrant...")
    upload_to_qdrant(
        qdrant_client, conf["collection_name"], embeddings, conf["upload_batch_size"]
    )
    ...
    faiss_distances, faiss_indices = search_faiss(
        faiss_index, query_embedding, conf["num_neighbours"]
    )
    ...
    qdrant_results = search_qdrant(
        qdrant_client, query_embedding, conf["collection_name"], conf["num_neighbours"]
    )
